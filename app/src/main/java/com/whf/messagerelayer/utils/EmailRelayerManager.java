...
public class EmailRelayerManager {
    ...
    //发送短信至目标邮件
    public static int relayEmail(NativeDataManager dataManager, SmsModel model) {
        Properties props = new Properties();
        User user = getSenderUser(dataManager);
        EmailMessage emailMessage = createEmailMessage(model, dataManager);
     ...
     * 封装消息实体
     */
    private static EmailMessage createEmailMessage(SmsModel model,
        ...
        message.setSubject("发件人: " + model.getAddress());
        message.setContent("<p>"+model.getContent()+"</p>\n" +
                "    <div style=\"text-align: right;\">\n" +
                "        时间: <time>"+model.getDatetime()+"</time>\n" +
                "    </div>");
